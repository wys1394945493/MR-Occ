...
class DinoVisionTransformer(nn.Module):
    ...
    def forward_features_list(self, x_list: List[Tensor], masks_list: List[Tensor]) -> List[Dict[str, Tensor]]:
        x = []
        rope = []
        for t_x, t_masks in zip(x_list, masks_list):                        # (6 3 256 704) None
            t2_x, hw_tuple = self.prepare_tokens_with_masks(t_x, t_masks)   # (6 709 1024) [16 44] rgb -> patch_embed(in=3 out=1024 kernel_size=(16 16) stride=(16 16))
            x.append(t2_x)
            rope.append(hw_tuple)
        for _, blk in enumerate(self.blocks):
            if self.rope_embed is not None:
                rope_sincos = [self.rope_embed(H=H, W=W) for H, W in rope]  # (704 64) (704 64)
            else:
                rope_sincos = [None for r in rope]
            x = blk(x, rope_sincos)                                         # (6 709 1024)
        all_x = x                                                           # (6 709 1024)
        output = []
        for idx, (x, masks) in enumerate(zip(all_x, masks_list)):
            if self.untie_cls_and_patch_norms or self.untie_global_and_local_cls_norm:
                ...
                x_norm = self.norm(x)                                   # (6 709 1024) # LayerNorm
                x_norm_cls_reg = x_norm[:, : self.n_storage_tokens + 1] # (6 5 1024)
                x_norm_patch = x_norm[:, self.n_storage_tokens + 1 :]   # (6 704 1024)
            output.append(
                {
    ...
    def _get_intermediate_layers_not_chunked(self, x: Tensor, n: int = 1) -> List[Tensor]:
        x, (H, W) = self.prepare_tokens_with_masks(x)   # (6 709 1024)
    ...
    def get_intermediate_layers(
        ...
        *,
        ...
        if norm:
            ...
            outputs = outputs_normed    # 24x[6 709 1024]
