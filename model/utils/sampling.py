...
def sampling_4d(
    ...
    sample_points_cam = torch.matmul(occ2img, sample_points).squeeze(
        -1
    )  # (B, T, N, Q, G*P, 4)
    ...
    sample_points_cam = sample_points_cam.reshape(
        B * T * G, Q, P, 3
    )  # (B*T*G, Q, P, 3)
    ...
    final = msmv_sampling(
        mlvl_feats, sample_points_cam, scale_weights
    )  # (B*T*G, Q, C, P)
