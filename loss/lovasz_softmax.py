...
def flatten_probas(probas, labels, ignore=None):
    ...
    if probas.dim() == 2:
        if ignore is not None:
            ...
            valid = ~torch.isin(labels, ignore)
    ...
    labels = labels.view(-1)  # (640000)
    if ignore is None:
        return probas, labels

    if not torch.is_tensor(ignore):
        ignore = torch.tensor(
            list(ignore) if isinstance(ignore, (list, tuple, set)) else [ignore],
            device=labels.device,
        )
    valid = ~torch.isin(labels, ignore)

    vprobas = probas[valid.nonzero().squeeze()]  # [39368 18]
    vlabels = labels[valid]  # [39368]
